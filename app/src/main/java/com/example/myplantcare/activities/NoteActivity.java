...
public class NoteActivity extends AppCompatActivity {
    ...
    private List<NoteSectionItem> categorizeNotes(List<Note> noteList) {
        // Tạo một Map để nhóm các note theo phần "7 ngày trước", "30 ngày trước", và các năm
        Map<String, List<Note>> groupedNotes = new HashMap<>();

        // Nhóm note theo phần ngày
        for (Note note : noteList) {
            String section = getDateSection(note.getDate());
            groupedNotes.computeIfAbsent(section, k -> new ArrayList<>()).add(note);
        }

        // Sắp xếp nhóm theo thứ tự: 7 ngày trước -> 30 ngày trước -> các năm giảm dần
        ...
        // Tạo danh sách để trả về với các note theo đúng thứ tự
        List<NoteSectionItem> noteSectionItems = new ArrayList<>();
        for (String section : sectionOrder) {
            noteSectionItems.add(new NoteSectionItem(section)); // Thêm header
            for (Note note : groupedNotes.get(section)) {
                noteSectionItems.add(new NoteSectionItem(note)); // Thêm note vào dưới mỗi header
            }
        }
